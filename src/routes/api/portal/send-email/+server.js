  ...
  try {
    ...
    try {
      ...
      if (transporter) {
        ...
        usingFallback = true;
        console.warn(`⚠️ Mailer unavailable on this host. Portal Access Link generated: ${portalLink}`);
      }
    } catch (err) {
      usingFallback = true;
      mailSendError = err.message;
      console.warn("Mail sending block failed, falling back to console log info:", err.message);
    }

    return json({ success: true, usingFallback, previewUrl, mailSendError });
  } catch (error) {
